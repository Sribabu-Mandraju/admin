        ...
        <div className="table-responsive  w-100  mt-4" style={{ height: "87vh", overflow: "scroll" }}>
          ...
          >
            <thead className="border" style={{ backgroundColor: " #F4F4F4"}}>
              <tr className=" bold-2" style={{ textWrap: "nowrap", color: "#006996",position:"sticky"}}>
                <td className="  ps-2" style={{ color: "#006996",width:"50px  " }}>
                  S.no
                </td>
            ...
            <tbody style={{backgroundColor:'#cce6ff'}}>
              {filteredData.map((item,index) => (
                <tr key={index} onClick={()=>handleRow(item.id)} className="  mt-2 py-2 bg-primary" style={{ testWrap: "nowrap",backgroundColor:"#cce6ff" }}>
                  <td className="p-3 ">{item.id}</td>
                    
                  <td className="p-3 " style={{ textWrap: "nowrap" }}>
                  {item.title}
                  </td>
                  <td className="p-3 ">{item.date}</td>
                  <td className="p-3 ">{item.postedBy}</td>
                  <td className="p-3 " ><p className={`m-2 badge text-white ${getStatusBadgeColor(item.status)}`}>{item.status}</p></td>
                </tr>
              ))}
            </tbody>
          </table>
