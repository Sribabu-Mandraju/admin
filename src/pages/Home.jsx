  ...
  return (
    ...
    <div className="d-flex justify-content-between align-items-center shadow w-100" style={{ height: '50px',backgroundColor:"#e6f2ff" }}>
      ...
      <div className="d-flex align-items-center">
        ...
        <button className=" bell-drop" style={{position:"relative",backgroundColor:"#e6f2ff",color:"black",fontSize:"",position:"relative",borderRadius:"",border:"none"}}>
          <TbBellRingingFilled className='mx-2 ' style={{fontSize:"20px",backgroundColor:""}} />
    ...
    <div className="w-100 d-flex" style={{ position: 'relative' }}>
     {
      width>1000?
      <>
      <div className="sidebar bg-white justify-content-between  d-flex flex-column shadow " style={{ height: '94vh', minWidth: '360px', marginLeft: side ? '0px' : '-360px',transitionDuration:"0.3s",zIndex:"200" }}>
          <div className="d-flex flex-column">
          
            {
              tabs.map((data)=> (
                <div className={`tab mb-2  ${tab== data.tab ?"tab-actived":""}`} onClick={() => {
                  setTab(data.tab)
                }}>
                  <div className="">
      ...
      <>
        <div className="sidebar justify-content-between  d-flex flex-column bg-white d-flex flex-column shadow " style={{ height: '100vh', minWidth: '100%', position:"absolute",left:side?'0px':'-100%',top:"-50px",transitionDuration:'0.3s',zIndex:"200" ,zIndex:"3"}}>
        <div className="d-flex flex-column">
        {width < 1000 ? (
          <>
            ...
            {
              tabs.map((data)=> (
                <div className={`tab mb-2  text-center ${tab== data.tab ?"":""}`} onClick={() => {
                  setTab(data.tab)
                  setSide(false)

                }}>
                  
                  <span style={{fontWeight:"700",fontSize:"20px",color:"#006996"}}>{data.tabName}</span>
                </div>
              ))
            }
          </div>
          <div className={`tab mb-2 mb-5 text-center `}  style={{backgroundColor:"#b3ecff",border:"2px solid black",borderRadius:"3px"}} onClick={()=> {
            navigate('/')
          }}>
                  <span className="px-3 b-5"><CiLogin /></span>
                  <span>logout</span>
      ...
      <div className="   shadow mx-2 mt-4 mb-2 py-2"  style={{width:"97%",height:width<700?"89vh":"85vh",minHeight:"85vh",overflowY:"scroll"}}>
